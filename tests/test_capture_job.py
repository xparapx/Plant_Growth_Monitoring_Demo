...
def test_job_never_publishes_fake_frames(context):
    ctx = context
    _setup(ctx)
    sent = []
    ctx.runner._publish = lambda payload, *a, **k: (sent.append(payload) or (True, ""))
    job = ctx.runner.run("dawn", trigger="test")
    assert job.state == "done", job.error
    assert job.ok_rows == 6 and job.fake and job.img_file.startswith("fake_")
    names = [s["name"] for s in job.steps]
    assert names.index("camera_open") < names.index("capture") < names.index("measure") < names.index("jsonl") < names.index("publish")
...
def test_debounce_force_and_if_missing(context):
    ...
    first = ctx.runner.run("dawn", allow_fake_publish=True)
