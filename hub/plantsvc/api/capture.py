...
def schedule(c=Depends(ctx)) -> dict[str, Any]:
    ...
    return {"tz": cfg.tz, "dawn": cfg.schedule.dawn, "pm": cfg.schedule.pm,
...
def capture_status(c=Depends(ctx)) -> dict[str, Any]:
    ...
    return {"job": running, "last": jobs[0] if jobs else None, "schedule": schedule(c),
            "camera": {"state": c.camera.state, "driver": c.camera.driver or c.camera.probe_driver(),
                       "preview": c.camera.status()["preview"]}}


@router.post("/capture/run", status_code=202)
async def capture_run(c=Depends(ctx), phase: str = "auto", wait: int = 0, force: int = 0,
                      allow_fake_publish: int = 0) -> dict[str, Any]:
    if c.runner.current():
    ...
    kwargs = dict(trigger="api", force=bool(force), allow_fake_publish=bool(allow_fake_publish))
    if wait:
        try:
