"""leaf_measure — 사진 1장 -> 화분별 캐노피 투영면적.
...
def leaf_mask(bgr, ksize=5, open_it=1, close_it=2):
    """ROI 한 칸 안에서 잎만 남긴 흑백 마스크와 픽셀 수.  -> (mask, px, blobs, otsu_thr)"""
...
def measure(img_path, phase, debug_dir=None, mask_dir=None, C: dict[str, Any] | None = None, *,
            ref_path=None, log=print) -> list[dict[str, Any]]:
    """Measure every ROI in one photo.  `C` is the legacy config dict; `ref_path` is calib.jpg.
    Returns one row per ROI (see run_collector growth columns + blobs/edge_frac diagnostics)."""
    if C is None:
        from ..settings import get_paths
        p = get_paths()
        C = json.load(open(p.config, encoding="utf-8"))
        ref_path = ref_path or p.calib
    ...
    if ppc <= 0:
        say("[WARN] px_per_cm_ref 가 없습니다 — area_cm2 는 null, ok=0 으로 기록됩니다")

    rois, drift = C.get("rois", []), None
    if ref_path and os.path.exists(str(ref_path)):
        drift = frame_align.align(img_path, ref_path, C, log=log)
        if drift["ok"] and drift["level"] == "drift":
            rois = frame_align.shift_rois(rois, drift["dx"], drift["dy"], img.shape)
        say(f"[ALIGN] {drift['level']}: {drift['msg']}")
    ...
    for roi in rois:
        ...
        if x < 0 or y < 0 or x + w > W or y + h > H:
            say(f'[WARN] {roi["plant_id"]}: ROI 가 프레임 밖 — 건너뜀')
            continue

        crop = img[y:y + h, x:x + w]
        mask, px, blobs, _thr = leaf_mask(crop)

        border = np.concatenate([mask[0, :], mask[-1, :], mask[:, 0], mask[:, -1]])
        edge_px = int((border > 0).sum())
        edge_frac = edge_px / float(2 * (w + h))
        tol = float(C.get("qc", {}).get("edge_tol", 0.02))
        edge = edge_frac > tol
        too_big = px > 0.9 * w * h
        ok = int(px > 0 and not edge and not too_big and ppc > 0 and not geom_bad)
        if edge:
            say(f'[WARN] {roi["plant_id"]}: 잎이 ROI 테두리의 {edge_frac * 100:.1f}%에 걸쳐 있습니다 '
                f'(허용 {tol * 100:.0f}%) — 면적이 <실제보다 작습니다>. 박스를 넓히세요')
        elif edge_px:
            say(f'[INFO] {roi["plant_id"]}: 테두리 접촉 {edge_frac * 100:.1f}% — 허용 범위')
        if too_big:
            say(f'[WARN] {roi["plant_id"]}: 마스크가 ROI 의 90% 초과 — 배경을 잡았을 가능성')
        ...
        if ppc > 0 and pot_cm > 0 and px > 0:
            ...
            if a_cm2 < foot * 0.5:
                say(f'[WARN] {roi["plant_id"]}: 면적 {a_cm2:.1f}cm² 가 화분 넓이 {foot:.0f}cm² 의 '
                    f'{a_cm2 / foot * 100:.0f}% 뿐입니다 — px_per_cm_ref({ppc:.1f}) 를 의심하세요')
            elif a_cm2 > foot * 4:
                say(f'[WARN] {roi["plant_id"]}: 면적 {a_cm2:.1f}cm² 가 화분 넓이의 '
        ...
        row = {"plant_id": roi["plant_id"], "treat": roi.get("treat"),
               ...
               "img_file": os.path.basename(str(img_path))}
        out.append(row)

        if mask_dir:
            os.makedirs(mask_dir, exist_ok=True)
            cv2.imwrite(os.path.join(str(mask_dir), f'{stem}_{roi["plant_id"]}.png'), mask)
        if debug_dir:
            os.makedirs(debug_dir, exist_ok=True)
            vis = crop.copy()
            vis[mask == 0] = (vis[mask == 0] * 0.25).astype(np.uint8)
            c, _ = cv2.findContours(mask, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
            cv2.drawContours(vis, c, -1, (0, 0, 255), 3)
            cv2.putText(vis, f'{roi["plant_id"]} {row["area_cm2"]}cm2{"" if ok else "  NG"}',
                        (12, 46), cv2.FONT_HERSHEY_SIMPLEX, 1.4,
                        (0, 255, 0) if ok else (0, 0, 255), 3)
            cv2.imwrite(os.path.join(str(debug_dir), f'{stem}_{roi["plant_id"]}.jpg'), vis)
    return out
...
def _cli(argv: list[str] | None = None) -> int:
    ...
    p = get_paths()
    path = argv[0] if argv else str(p.calib)
    phase = argv[1] if len(argv) > 1 else "test"
    rows = measure(path, phase, p.debug, p.mask)
    ...
    print(f"\n  오버레이를 <반드시 눈으로> 확인하세요: {p.debug}\n"
          f"  브라우저: http://<pi>:8080/api/images?kind=debug\n")
    return 0
