      ...
      {q.data && q.data.pots.length > 0 && (
        <div className="flex flex-col gap-4">
          {Object.entries(groups).map(([treat, pots]) => (
            <div key={treat}>
              <div className="label mb-2" style={{ color: trtVar(treat, 'ink') }}>{TREAT_NAME[treat]}</div>
  ...
  return (
    <m.div initial={{ scale: 0.92, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} transition={{ duration: 0.3, ease: [0.2, 0.8, 0.2, 1] }} className="min-w-0">
      <div className="flex items-baseline gap-2 text-[12px]">
        <b className="num">{s.plant_id.toUpperCase()}</b>
        <span className="num font-bold" style={{ color: trtVar(s.treat, 'ink') }}>{gain === null ? '—' : `${gain >= 0 ? '+' : ''}${fmtNum(gain, 0)}%`}</span>
        <span className="num text-[10.5px] text-muted">{fmtNum(s.gap_d, 1)}일</span>
      </div>
      <svg viewBox={`${-lim} ${-lim} ${2 * lim} ${2 * lim}`} className="mt-1 aspect-square w-full" role="img" aria-label={`${s.plant_id} 캐노피 실루엣 어제 대비 오늘 ${gain === null ? '' : fmtNum(gain, 0) + '%'}`}>
        <polygon points={pts(s.old.contour)} fill={trtVar(s.treat, 'fill')} opacity="0.34" />
        <polygon points={pts(s.new.contour)} fill={trtVar(s.treat, 'fill')} opacity="0.72" stroke={trtVar(s.treat, 'ink')} strokeWidth={lim * 0.012} strokeLinejoin="round" />
      </svg>
