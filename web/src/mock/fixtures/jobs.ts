...
export function newJob(phase: 'dawn' | 'pm', trigger: string, startMs: number, imgFile: string): Job {
  return {
    ...
    img_file: imgFile, n_rows: 0, ok_rows: 0, published: false, error: null, steps: [], rows: [],
    fake: false, drift: null, log: [], publish_error: null,
  }
}
...
export function failedJob(now: number): Job {
  ...
  const names = [...STEP_NAMES.slice(0, 7), 'failed']
  const waits = [0, 180, 210, 320, 190, 2100, 110, 640]
  let t = t0
  ...
  j.log = [`[${j.img_file}] shot ${j.img_file}`, '[measure] p4: no contour — mask empty']
  return j
}
